...
<div  className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-20 p-3 ' >
    {cardData.map(data => (
    <div key={data.id} className='h-82 lg:w-80  rounded-lg shadow-lg hover:bg-orange-100 border-black w-full flex flex-col'>
   <div className='w-full h-48 '><img className="  bg-center bg-cover h-[100%] w-full object-cover "src={data.img} alt="" /></div>
   
  <div className='flex flex-row justify-between w-full p-2'>
  <div className='flex flex-col  space-y-1 lg:h-32 w-full'><div className='lg:h-28 '><h1 className='font-bold text-xl'>{data.Dishes}</h1>
